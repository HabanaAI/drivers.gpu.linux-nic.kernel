...
#define NIC_IS_PSN_CYCLIC_BIG(_psn_a, _psn_b)				\
...
#define NIC_SET(desc, idx, shift, val, len) \
		((desc).data[idx] |= (u64) ((val) & BITMLL(len)) << (shift))

#define NIC_CLEAR(desc, idx, shift, val, len) \
		((desc).data[idx] &= ~((u64) (BITMLL(len)) << (shift)))

#define NIC_SET_BITS(desc, lsb, val, len)			\
	do {							\
		BUILD_BUG_ON(((lsb) / 64) != (((lsb) + (len) - 1) / 64)); \
		NIC_CLEAR(desc, (lsb) / 64, (lsb) % 64, val, len); \
		NIC_SET(desc, (lsb) / 64, (lsb) % 64, val, len);	\
	} while (0)
...
#define NIC_GET_BITS(desc, lsb, len)			\
	({							\
		BUILD_BUG_ON(((lsb) / 64) != (((lsb) + (len) - 1) / 64)); \
		NIC_GET(desc, (lsb) / 64, (lsb) % 64, len);	\
	})
...
struct gaudi2_en_aux_data {
	struct hl_cn_ring **rx_rings;
	struct hl_cn_ring **cq_rings;
	struct hl_cn_ring **wq_rings;
	u32 kernel_asid;
	u32 raw_qpn;
	u32 tx_ring_len;
	u32 schedq_num;
...
struct gaudi2_en_aux_ops {
	/* en2cn */
	void (*configure_cq)(struct hl_aux_dev *aux_dev, u32 port, u32 coalesce_usec, bool enable);
	void (*arm_cq)(struct hl_aux_dev *aux_dev, u32 port, u32 index);
	void (*write_rx_ci)(struct hl_aux_dev *aux_dev, u32 port, u32 ci);
	void (*get_pfc_cnts)(struct hl_aux_dev *aux_dev, u32 port, int pfc_prio,
				u64 *indications, u64 *requests);
	int (*ring_tx_doorbell)(struct hl_aux_dev *aux_dev, u32 port, u32 pi, bool *full_after_tx);
	char* (*qp_err_syndrome_to_str)(u32 syndrome);
	void (*db_fifo_reset)(struct hl_aux_dev *aux_dev, u32 port);

	/* cn2en */
	int (*port_reset_locked)(struct hl_aux_dev *aux_dev, u32 port);
	u32 (*get_overrun_cnt)(struct hl_aux_dev *aux_dev, u32 port_idx);
};
