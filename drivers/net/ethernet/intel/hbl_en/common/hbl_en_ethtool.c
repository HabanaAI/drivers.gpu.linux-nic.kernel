...
static int hbl_en_set_autoneg(struct hbl_en_port *port, bool enable)
{
	struct hbl_en_device *hdev = port->hdev;
	int rc = 0;

	if (port->auto_neg_enable == enable)
		return 0;

	if (atomic_cmpxchg(&port->in_reset, 0, 1)) {
		netdev_err(port->ndev, "port is in reset, can't update autoneg\n");
		return -EBUSY;
	}

	if (enable && !(hdev->auto_neg_mask & BIT(port->idx))) {
		netdev_err(port->ndev, "port autoneg is disabled by BMC\n");
		rc = -EOPNOTSUPP;
		goto out;
	}

	port->auto_neg_enable = enable;

	if (netif_running(port->ndev)) {
		rc = hbl_en_port_reset(port);
		if (rc) {
			netdev_err(port->ndev, "Failed to reset port for autoneg update\n");
			goto out;
		}
	}
...
static void hbl_en_get_pauseparam(struct net_device *ndev, struct ethtool_pauseparam *pause)
{
	struct hbl_en_port *port = hbl_netdev_priv(ndev);

	/* we don't support LFC */
	pause->rx_pause = 0;
	pause->tx_pause = 0;

	pause->autoneg = port->auto_neg_enable ? AUTONEG_ENABLE : AUTONEG_DISABLE;
}

static int hbl_en_set_pauseparam(struct net_device *ndev, struct ethtool_pauseparam *pause)
{
	struct hbl_en_port *port = hbl_netdev_priv(ndev);

	if (pause->rx_pause || pause->tx_pause) {
		netdev_err(ndev, "Link flow control is not supported\n");
		return -EOPNOTSUPP;
	}

	return hbl_en_set_autoneg(port, pause->autoneg == AUTONEG_ENABLE);
}
					     ...
					     struct ethtool_link_ksettings *cmd)
{
	...
	if (hdev->auto_neg_mask & BIT(port->idx)) {
		ethtool_link_ksettings_add_link_mode(cmd, supported, Autoneg);
		if (port->auto_neg_enable) {
			ethtool_link_ksettings_add_link_mode(cmd, advertising, Autoneg);
			cmd->base.autoneg = AUTONEG_ENABLE;
			if (aux_ops->is_autoneg_completed(aux_dev, port_idx))
...
hbl_en_ethtool_set_link_ksettings(struct net_device *ndev, const struct ethtool_link_ksettings *cmd)
{
	...
	return hbl_en_set_autoneg(port, cmd->base.autoneg == AUTONEG_ENABLE);
}
...
static const struct ethtool_ops hbl_en_ethtool_ops_coalesce = {
	...
	.get_pauseparam = hbl_en_get_pauseparam,
	.set_pauseparam = hbl_en_set_pauseparam,
};
