...
const struct hbl_cn_stat hbl_cn_mac_fec_stats[] = {
	{"correctable_errors", 0x2, 0x3},
	{"uncorrectable_errors", 0x4, 0x5}
};

const struct hbl_cn_stat hbl_cn_mac_stats_rx[] = {
	{"Octets", 0x0},
...
const struct hbl_cn_stat hbl_cn_mac_stats_tx[] = {
	{"Octets", 0x0},
...
static const size_t pcs_counters_str_len = ARRAY_SIZE(pcs_counters_str);
const size_t hbl_cn_mac_fec_stats_len = ARRAY_SIZE(hbl_cn_mac_fec_stats);
const size_t hbl_cn_mac_stats_rx_len = ARRAY_SIZE(hbl_cn_mac_stats_rx);
const size_t hbl_cn_mac_stats_tx_len = ARRAY_SIZE(hbl_cn_mac_stats_tx);
...
static struct hbl_cn_device *hbl_cn_aux2nic(struct hbl_aux_dev *aux_dev)
{
	return aux_dev->type == HBL_AUX_DEV_ETH ?
	       container_of(aux_dev, struct hbl_cn_device, en_aux_dev) :
	       container_of(aux_dev, struct hbl_cn_device, ib_aux_dev);
}
...
{
	struct hbl_cn_device *hdev = hbl_cn_aux2nic(aux_dev);
...
static bool hbl_cn_is_autoneg_completed(struct hbl_aux_dev *aux_dev, u32 port)
{
	struct hbl_cn_device *hdev = hbl_cn_aux2nic(aux_dev);
	struct hbl_cn_port *cn_port;
	...
	return cn_port->auto_neg_resolved;
}
...
static int hbl_cn_ib_alloc_ucontext(struct hbl_aux_dev *ib_aux_dev, int user_fd, void **cn_ib_ctx)
{
	...
	struct hbl_cn_device *hdev = hbl_cn_aux2nic(ib_aux_dev);
	struct hbl_aux_dev *aux_dev = hdev->cn_aux_dev;
	struct hbl_cn_asic_funcs *asic_funcs;
	struct hbl_cn_aux_ops *aux_ops;
...
static int hbl_cn_en_aux_data_init(struct hbl_cn_device *hdev)
{
	...
	en_aux_ops->is_autoneg_completed = hbl_cn_is_autoneg_completed;
			  ...
			  struct hbl_cni_req_conn_ctx_out *out)
{
	...
	if (wq_size > max_wq_size) {
		dev_dbg(hdev->dev, "Port %d, Requester QP - requested size (%d) > max size (%d)\n",
			port, wq_size, max_wq_size);
		return -EINVAL;
	}
