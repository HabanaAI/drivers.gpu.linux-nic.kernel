...
{
	struct hbl_cn_device *hdev = gaudi2_port->hdev;
	u32 rc, val, port = gaudi2_port->cn_port->port;
	...
	rc = read_poll_timeout(NIC_MACRO_RREG32, val,
			       !(val & NIC0_MAC_CH0_MAC_PCS_CONTROL1_FLD_RESET_MASK), 200,
			       hdev->pending_reset_long_timeout * 1000ull, true,
			       NIC0_MAC_CH0_MAC_PCS_CONTROL1 + MAC_CH_OFFSET(lane));
	if (rc)
			      ...
			      u64 ci_device_handle, struct hbl_cn_db_fifo_xarray_pdata *xa_pdata)
{
	...
	val = ctx->asid;
	val |= 1 << NIC0_QPC0_DB_FIFO_USER_OVRD_MMU_BYPASS_SHIFT;
