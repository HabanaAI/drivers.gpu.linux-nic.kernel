...
#define NIC_STAT(m) {#m, offsetof(struct hl_en_port, net_stats.m)}

static struct hl_cn_stat netdev_eth_stats[] = {
	NIC_STAT(rx_packets),
	NIC_STAT(tx_packets),
	NIC_STAT(rx_bytes),
	NIC_STAT(tx_bytes),
...
static void hl_en_ethtool_get_drvinfo(struct net_device *ndev, struct ethtool_drvinfo *drvinfo)
{
	struct hl_en_device *hdev;
	struct hl_en_port *port;

	port = hl_netdev_priv(ndev);
	hdev = port->hdev;
	...
	if (hdev->pdev)
		...
		snprintf(drvinfo->bus_info, sizeof(drvinfo->bus_info), "0000:00:%02d.0",
			 hdev->core_dev_minor);
}

static int hl_en_ethtool_get_module_info(struct net_device *ndev, struct ethtool_modinfo *modinfo)
{
...
static int hl_en_ethtool_get_module_eeprom(struct net_device *ndev, struct ethtool_eeprom *ee,
					   u8 *data)
{
	struct hl_en_device *hdev;
	struct hl_en_port *port;
...
static u32 hl_en_ethtool_get_priv_flags(struct net_device *ndev)
{
	struct hl_en_port *port = hl_netdev_priv(ndev);

	return port->pflags;
}

static int hl_en_ethtool_set_priv_flags(struct net_device *ndev, u32 priv_flags)
{
	struct hl_en_port *port = hl_netdev_priv(ndev);
...
static int hl_en_ethtool_get_link_ksettings(struct net_device *ndev,
...
static int
hl_en_ethtool_set_link_ksettings(struct net_device *ndev, const struct ethtool_link_ksettings *cmd)
{
	...
	rc = hl_en_ethtool_get_link_ksettings(ndev, &curr_cmd);
	if (rc)
		return rc;
...
static int hl_en_ethtool_get_sset_count(struct net_device *ndev, int sset)
{
	struct hl_en_port *port = hl_netdev_priv(ndev);
...
static void hl_en_ethtool_get_strings(struct net_device *ndev, u32 stringset, u8 *data)
{
	struct hl_en_port *port = hl_netdev_priv(ndev);
	struct hl_en_device *hdev = port->hdev;
	struct hl_en_aux_ops *aux_ops;
	...
	case ETH_SS_STATS:
		for (i = 0; i < netdev_eth_stats_len; i++)
			memcpy(data + i * ETH_GSTRING_LEN, netdev_eth_stats[i].str,
			       ETH_GSTRING_LEN);
	...
	case ETH_SS_PRIV_FLAGS:
		for (i = 0; i < pflags_str_len; i++)
			strcpy(data + i * ETH_GSTRING_LEN, pflags_str[i]);
		break;
	}
}

static void hl_en_ethtool_get_ethtool_stats(struct net_device *ndev,
					    __always_unused struct ethtool_stats *stats, u64 *data)
{
	 ...
	 * much better than adding a lock to sychronise and increase the overhead of the Rx/Tx
...
static int hl_en_ethtool_get_coalesce(struct net_device *ndev,
...
static int hl_en_ethtool_set_coalesce(struct net_device *ndev,
				      struct ethtool_coalesce *coal,
				      struct kernel_ethtool_coalesce *kernel_coal,
				      struct netlink_ext_ack *extack)
{
	...
	/* TODO: find the max num of pending XMITs that do not deplete the skbs of stack */
...
static const struct ethtool_ops hl_en_ethtool_ops = {
	.supported_coalesce_params = ETHTOOL_COALESCE_RX_USECS | ETHTOOL_COALESCE_RX_MAX_FRAMES |
				     ETHTOOL_COALESCE_TX_MAX_FRAMES,
	.get_drvinfo = hl_en_ethtool_get_drvinfo,
	.get_link = ethtool_op_get_link,
	.get_module_info = hl_en_ethtool_get_module_info,
	.get_module_eeprom = hl_en_ethtool_get_module_eeprom,
	.get_priv_flags = hl_en_ethtool_get_priv_flags,
	.set_priv_flags = hl_en_ethtool_set_priv_flags,
	.get_link_ksettings = hl_en_ethtool_get_link_ksettings,
	.set_link_ksettings = hl_en_ethtool_set_link_ksettings,
	.get_sset_count = hl_en_ethtool_get_sset_count,
	.get_strings = hl_en_ethtool_get_strings,
	.get_ethtool_stats = hl_en_ethtool_get_ethtool_stats,
	.get_coalesce = hl_en_ethtool_get_coalesce,
	.set_coalesce = hl_en_ethtool_set_coalesce,
};

const struct ethtool_ops *hl_en_ethtool_get_ops(void)
{
	return &hl_en_ethtool_ops;
}
