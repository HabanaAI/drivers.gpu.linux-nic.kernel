...
/**
 * struct hl_en_tx_pkt_work - used to schedule a work of a Tx packet.
 ...
 */
struct hl_en_tx_pkt_work {
	struct work_struct tx_work;
	struct hl_en_port *port;
	struct sk_buff *skb;
};

static int hl_en_napi_poll(struct napi_struct *napi, int budget);
static int hl_en_port_open(struct hl_en_port *port);

static int hl_en_ports_reopen(struct hl_aux_dev *aux_dev)
{
	...
	for (i = 0; i < hdev->max_num_of_ports; i++) {
		 ...
		 * here anyway.
		 * See hl_en_ports_stop_prepare() for more info.
		 */
...
static void hl_en_port_fini(struct hl_en_port *port)
{
	if (port->rx_wq)
...
static int hl_en_port_init(struct hl_en_port *port)
{
	...
	if (hdev->is_threaded_tx) {
		...
		snprintf(wq_name, sizeof(wq_name) - 1, "hl%u-port%d-tx-wq", hdev->core_dev_id,
			 port_idx);
		port->tx_wq = create_singlethread_workqueue(wq_name);
		if (!port->tx_wq) {
			dev_err(hdev->dev, "Failed to allocate Tx WQ\n");
			return -ENOMEM;
		}
	}
	...
	if (hdev->poll_enable) {
		memset(wq_name, 0, sizeof(wq_name));
		snprintf(wq_name, sizeof(wq_name) - 1, "hl%u-port%d-rx-wq", hdev->core_dev_id,
			 port_idx);
		port->rx_wq = create_singlethread_workqueue(wq_name);
		if (!port->rx_wq) {
	...
	hl_en_ethtool_init_coalesce(port);

	return 0;

fail:
	hl_en_port_fini(port);

	return rc;
}

static void _hl_en_set_port_status(struct hl_en_port *port, bool up)
{
...
static void hl_en_set_port_status(struct hl_aux_dev *aux_dev, u32 port_idx, bool up)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);

	_hl_en_set_port_status(port, up);
}

static bool hl_en_is_port_open(struct hl_aux_dev *aux_dev, u32 port_idx)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);
...
static int hl_en_get_src_ip(struct hl_aux_dev *aux_dev, u32 port_idx, u32 *src_ip)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);
	struct net_device *ndev = port->ndev;
	...
	*src_ip = 0;
	...
	while (ifa) {
		if (!strcmp(ndev->name, ifa->ifa_label)) {
			/* covnert the BE to native and later on it will be
...
static void hl_en_reset_stats(struct hl_aux_dev *aux_dev, u32 port_idx)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);
...
static u32 hl_en_get_mtu(struct hl_aux_dev *aux_dev, u32 port_idx)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);
	struct net_device *ndev = port->ndev;
	u32 mtu;
...
static u32 hl_en_get_pflags(struct hl_aux_dev *aux_dev, u32 port_idx)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);
...
static void hl_en_set_dev_lpbk(struct hl_aux_dev *aux_dev, u32 port_idx, bool enable)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);
	struct net_device *ndev = port->ndev;
...
static int hl_en_port_open_locked(struct hl_en_port *port)
{
	struct hl_en_device *hdev = port->hdev;
	struct net_device *ndev = port->ndev;
	...
	if (!hdev->poll_enable)
		netif_napi_add(ndev, &port->napi, hl_en_napi_poll);
...
static int hl_en_port_open(struct hl_en_port *port)
{
	...
	rc = hl_en_port_open_locked(port);
	aux_ops->ctrl_unlock(aux_dev, port_idx);

	return rc;
}

static int hl_en_open(struct net_device *netdev)
{
...
/* This function should be called after ctrl_lock was taken */
static void hl_en_port_close_locked(struct hl_en_port *port)
{
	struct hl_en_device *hdev = port->hdev;
	struct hl_en_aux_ops *aux_ops;
	struct hl_aux_dev *aux_dev;
	...
	/* flush any threaded Tx packets */
	if (hdev->is_threaded_tx)
		drain_workqueue(port->tx_wq);
...
static void hl_en_port_close(struct hl_en_port *port)
{
	struct hl_en_device *hdev = port->hdev;
	struct hl_en_aux_ops *aux_ops;
	struct hl_aux_dev *aux_dev;
	...
	aux_ops->ctrl_lock(aux_dev, port_idx);
	hl_en_port_close_locked(port);
	aux_ops->ctrl_unlock(aux_dev, port_idx);
}

/* This function should be called after ctrl_lock was taken */
static int __hl_en_port_reset_locked(struct hl_en_port *port)
{
	hl_en_port_close_locked(port);

	return hl_en_port_open_locked(port);
}
...
int hl_en_port_reset_locked(struct hl_aux_dev *aux_dev, u32 port_idx)
{
	...
	return __hl_en_port_reset_locked(port);
}
...
int hl_en_port_reset(struct hl_en_port *port)
{
	...
	aux_ops->ctrl_lock(aux_dev, port_idx);
	if (hdev->sleep_after_port_close) {
		hl_en_port_close_locked(port);
		aux_ops->ctrl_unlock(aux_dev, port_idx);
		...
		rc = __hl_en_port_reset_locked(port);
	}
	aux_ops->ctrl_unlock(aux_dev, port_idx);
...
static int hl_en_close(struct net_device *netdev)
{
	struct hl_en_port *port = hl_netdev_priv(netdev);
	struct hl_en_device *hdev = port->hdev;
	ktime_t timeout;
...
/**
 * hl_en_ports_stop_prepare() - stop the Rx and Tx and synchronize with other reset flows.
 ...
 */
static void hl_en_ports_stop_prepare(struct hl_aux_dev *aux_dev)
{
	struct hl_en_device *hdev = aux_dev->priv;
	struct hl_en_port *port;
	ktime_t timeout;
...
static void hl_en_ports_stop(struct hl_aux_dev *aux_dev)
{
	struct hl_en_device *hdev = aux_dev->priv;
	struct hl_en_port *port;
	int i;
...
static int hl_en_change_mtu(struct net_device *netdev, int new_mtu)
{
	struct hl_en_port *port = hl_netdev_priv(netdev);
	struct hl_en_device *hdev = port->hdev;
	int rc = 0;
...
static inline void swap_l3(char *buf)
{
	...
	/* skip the Ehternet header and the IP header till source IP address */
	buf += ETH_HLEN + 12;
	tmp = ((u32 *)buf)[0];
	((u32 *)buf)[0] = ((u32 *)buf)[1];
	((u32 *)buf)[1] = tmp;
...
static void do_tx_swap(struct hl_en_port *port, struct sk_buff *skb)
{
	struct hl_en_device *hdev = port->hdev;
...
static bool is_pkt_swap_enabled(struct hl_en_device *hdev)
{
	struct hl_en_aux_ops *aux_ops;
	struct hl_aux_dev *aux_dev;
...
static bool is_tx_disabled(struct hl_en_port *port)
{
	struct hl_en_device *hdev = port->hdev;
	struct hl_en_aux_ops *aux_ops;
	struct hl_aux_dev *aux_dev;
...
static netdev_tx_t hl_en_handle_tx(struct hl_en_port *port, struct sk_buff *skb)
{
	struct hl_en_device *hdev = port->hdev;
	netdev_tx_t ret;
...
static void hl_en_tx_pkt_handler(struct work_struct *work)
{
	struct hl_en_tx_pkt_work *tx_work = container_of(work, struct hl_en_tx_pkt_work, tx_work);
	struct hl_en_port *port = tx_work->port;

	hl_en_handle_tx(port, tx_work->skb);

	kfree(tx_work);
}

static netdev_tx_t hl_en_queue_tx(struct hl_en_port *port, struct sk_buff *skb)
{
	struct hl_en_tx_pkt_work *tx_work;

	tx_work = kmalloc(sizeof(*tx_work), GFP_ATOMIC);
	if (tx_work) {
		INIT_WORK(&tx_work->tx_work, hl_en_tx_pkt_handler);
		tx_work->port = port;
		tx_work->skb = skb;

		queue_work(port->tx_wq, &tx_work->tx_work);
	}

	return NETDEV_TX_OK;
}

static netdev_tx_t hl_en_start_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	struct hl_en_port *port = hl_netdev_priv(netdev);
	...
	if (hdev->is_threaded_tx)
		return hl_en_queue_tx(port, skb);
	else
		return hl_en_handle_tx(port, skb);
}

static int hl_en_set_port_mac_loopback(struct hl_en_port *port, bool enable)
{
	struct hl_en_device *hdev = port->hdev;
	struct net_device *ndev = port->ndev;
	struct hl_en_aux_ops *aux_ops;
...
static int hl_en_set_features(struct net_device *netdev, netdev_features_t features)
{
	struct hl_en_port *port = hl_netdev_priv(netdev);
	...
	if (changed & NETIF_F_LOOPBACK)
		rc = hl_en_set_port_mac_loopback(port, !!(features & NETIF_F_LOOPBACK));
...
static void hl_en_handle_tx_timeout(struct net_device *netdev, unsigned int txqueue)
{
	struct hl_en_port *port = hl_netdev_priv(netdev);
...
static void hl_en_get_stats64(struct net_device *dev, struct rtnl_link_stats64 *stats)
{
	struct hl_en_port *port = hl_netdev_priv(dev);
...
static const struct net_device_ops hl_en_netdev_ops = {
	.ndo_open = hl_en_open,
	.ndo_stop = hl_en_close,
	.ndo_start_xmit = hl_en_start_xmit,
	.ndo_validate_addr = eth_validate_addr,
	.ndo_change_mtu = hl_en_change_mtu,
	.ndo_set_features = hl_en_set_features,
	.ndo_get_stats64 = hl_en_get_stats64,
	.ndo_tx_timeout = hl_en_handle_tx_timeout,
};

static void hl_en_set_ops(struct net_device *ndev)
{
	ndev->netdev_ops = &hl_en_netdev_ops;
	ndev->ethtool_ops = hl_en_ethtool_get_ops();
#ifdef CONFIG_DCB
	ndev->dcbnl_ops = &hl_en_dcbnl_ops;
#endif
}

static int hl_en_port_register(struct hl_en_port *port)
{
	struct hl_en_device *hdev = port->hdev;
	struct hl_en_aux_ops *aux_ops;
	struct hl_aux_dev *aux_dev;
	u32 port_idx = port->idx;
	struct hl_en_port **ptr;
	...
	ndev = alloc_etherdev(sizeof(struct hl_en_port *));
	if (!ndev) {
	...
	*ptr = port;
	 ...
	 * only if there are more packets to be processed. Else we reenable the CQ Arm interrupt
	 * and exit the poll.
	 */
	if (hdev->poll_enable)
		hl_en_rx_poll_trigger_init(port);
...
static void dump_swap_pkt(struct hl_en_port *port, struct sk_buff *skb)
{
	struct hl_en_device *hdev = port->hdev;
	u16 *tmp_buff = (u16 *)skb->data;
	u32 port_idx = port->idx;
...
static bool __hl_en_rx_poll_schedule(struct hl_en_port *port, unsigned long delay)
{
	return queue_delayed_work(port->rx_wq, &port->rx_poll_work, delay);
}

static void hl_en_rx_poll_work(struct work_struct *work)
{
	struct hl_en_port *port = container_of(work, struct hl_en_port, rx_poll_work.work);
	struct hl_en_device *hdev = port->hdev;
	int pkt_count;

	pkt_count = hl_en_handle_rx(port, NAPI_POLL_WEIGHT);

	/* Reschedule the poll if we have consumed budget which means we still have packets to
	 * process. Else Reenable the Rx IRQs and exit the work
	 */
	if (pkt_count < NAPI_POLL_WEIGHT)
		hdev->asic_funcs.reenable_rx_irq(port);
	else
		__hl_en_rx_poll_schedule(port, 0);
}
...
{
	INIT_DELAYED_WORK(&port->rx_poll_work, hl_en_rx_poll_work);
}

bool hl_en_rx_poll_start(struct hl_en_port *port)
{
	return __hl_en_rx_poll_schedule(port, msecs_to_jiffies(1));
}
...
static int hl_en_napi_poll(struct napi_struct *napi, int budget)
{
	struct hl_en_port *port = container_of(napi, struct hl_en_port, napi);
	struct hl_en_device *hdev = port->hdev;
	int pkt_count;
	...
	pkt_count = hl_en_handle_rx(port, budget);
...
static void hl_en_port_unregister(struct hl_en_port *port)
{
	struct net_device *ndev = port->ndev;
...
static int hl_en_set_asic_funcs(struct hl_en_device *hdev)
{
...
static struct net_device *hl_en_get_netdev(struct hl_aux_dev *aux_dev, u32 port_idx)
{
	struct hl_en_port *port = HL_EN_PORT(aux_dev, port_idx);
...
static void hl_en_handle_eqe(struct hl_aux_dev *aux_dev, u32 port, struct hl_cn_eqe *eqe)
{
	struct hl_en_device *hdev = aux_dev->priv;
...
static void hl_en_set_aux_ops(struct hl_en_device *hdev, bool enable)
{
	struct hl_en_aux_ops *aux_ops = hdev->aux_dev->aux_ops;

	if (enable) {
		aux_ops->ports_reopen = hl_en_ports_reopen;
		aux_ops->ports_stop_prepare = hl_en_ports_stop_prepare;
		aux_ops->ports_stop = hl_en_ports_stop;
		aux_ops->set_port_status = hl_en_set_port_status;
		aux_ops->is_port_open = hl_en_is_port_open;
		aux_ops->get_src_ip = hl_en_get_src_ip;
		aux_ops->reset_stats = hl_en_reset_stats;
		aux_ops->get_mtu = hl_en_get_mtu;
		aux_ops->get_pflags = hl_en_get_pflags;
		aux_ops->set_dev_lpbk = hl_en_set_dev_lpbk;
		aux_ops->get_netdev = hl_en_get_netdev;
		aux_ops->handle_eqe = hl_en_handle_eqe;
	} else {
...
int hl_en_dev_init(struct hl_en_device *hdev)
{
	...
	/* init the function pointers here before calling hl_en_port_register which sets up
	...
	for (i = 0; i < hdev->max_num_of_ports; i++, port_cnt++) {
		...
		rc = hl_en_port_init(port);
		if (rc) {
			dev_err(hdev->dev, "port init failed\n");
			goto unregister_ports;
		}

		rc = hl_en_port_register(port);
		if (rc) {
...
unregister_ports:
	for (i = 0; i < port_cnt; i++) {
		...
		hl_en_port_unregister(port);
		hl_en_port_fini(port);
	}

	hl_en_set_aux_ops(hdev, false);
