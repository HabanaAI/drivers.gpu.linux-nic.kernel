...
static void gaudi2_cn_link_event_handler(struct gaudi2_cn_port *gaudi2_port)
{
	...
	struct hl_cn_port *cn_port_curr;
	...
	for (l = 0; l < NIC_MAC_LANES; l++) {
		 ...
		 */
		if (!hl_cn_is_port_open(cn_port_curr)) {
		...
		if (prev_link_up != link_up && !link_up) {
			...
			if (gaudi2_port_curr->qp_destroy_cnt && !cn_port_curr->mac_loopback) {
				...
				gaudi2_cn_hw_mac_loopback_cfg(gaudi2_port_curr, true);
		...
		if (!hdev->phy_config_fw || cn_port_curr->phy_fw_tuned) {
			...
			hl_cn_phy_set_port_status(cn_port_curr, link_up);
		}
...
static void gaudi2_cn_eq_dispatcher_default_handler(struct gaudi2_cn_port *gaudi2_port)
{
	...
	while (!hl_cn_eq_dispatcher_dequeue(cn_port, hdev->kernel_asid, &eqe, true)) {
		if (!EQE_IS_VALID(&eqe)) {
		...
		case EQE_COMP_ERR:
			dev_warn_ratelimited(hdev->dev,
					     "Port-%d cq-err event for invalid CQ:%d\n",
					     port, EQE_CQ_EVENT_CQ_NUM(&eqe));
			break;
...
static void cn_eq_handler(struct gaudi2_cn_port *gaudi2_port)
{
	...
	while (eq_ring->ci_shadow != eq_ring->pi_shadow) {
		eqe_p = (struct hl_cn_eqe *)RING_BUF_ADDRESS(eq_ring) +
		...
		} else if ((event_type != EQE_COMP_ERR) ||
			   !gaudi2_cn_is_cq_in_overrun(cn_port, EQE_CQ_EVENT_CQ_NUM(eqe_p))) {
			rc = hl_cn_eq_dispatcher_enqueue(cn_port, eqe_p);
			if (rc)
				dev_warn_ratelimited(hdev->dev,
...
irqreturn_t gaudi2_cn_eq_irq_handler(struct hl_aux_dev *aux_dev, int irq)
{
	...
	gaudi2 = hdev->asic_specific;
	gaudi2_port = &gaudi2->cn_ports[irq];
...
{
	struct hl_cn_ring *ring = &gaudi2_port->eq_ring;
	struct hl_cn_device *hdev = gaudi2_port->hdev;
	u32 port = gaudi2_port->cn_port->port;
						      ...
						      const struct hl_cn_eqe *eqe)
{
	...
	case EQE_COMP_ERR:
		...
		dq = hl_cn_cqn_to_dq(ev_dqs, cqn, gaudi2_port->hdev);
		break;
	case EQE_QP_ERR:
		qpn = EQE_QP_EVENT_QPN(eqe);
		dq = hl_cn_qpn_to_dq(ev_dqs, qpn);
		break;
	case EQE_RAW_TX_COMP:
	...
	case EQE_DB_FIFO_OVERRUN:
		...
		dq = hl_cn_dbn_to_dq(ev_dqs, dbn, gaudi2_port->hdev);
		break;
	case EQE_CONG:
		ccqn = EQE_CQ_EVENT_CCQ_NUM(eqe);
		dq = hl_cn_ccqn_to_dq(ev_dqs, ccqn, gaudi2_port->hdev);
		break;
	case EQE_QP_ALIGN_COUNTERS:
...
int gaudi2_cn_eq_dispatcher_register_db(struct gaudi2_cn_port *gaudi2_port, u32 asid, u32 dbn)
{
	...
	return hl_cn_eq_dispatcher_register_db(gaudi2_port->cn_port, asid, dbn);
}
