...
#define NIC_PHY_MAC_REMOTE_FAULT_CNT		20
...
static int get_tx_lane_in_macro(struct hl_cn_device *hdev, u32 port, int lane)
{
...
static void get_tx_port_and_lane(struct hl_cn_device *hdev, u32 port, int lane, u32 *tx_port,
				 int *tx_lane)
{
...
static bool is_lane_swapping(struct hl_cn_device *hdev, u32 port, int lane)
{
	u32 tx_port;
	int tx_lane;
...
static void set_fw_lane_mapping(struct hl_cn_device *hdev)
{
...
static void mac_lane_remap(struct hl_cn_device *hdev, u32 port)
{
	if (hdev->mac_lane_remap[port >> 1])
		NIC_MACRO_WREG32(NIC0_PHY_PHY_ASYNC_LANE_SWAP, hdev->mac_lane_remap[port >> 1]);
}

static void soft_reset(struct hl_cn_device *hdev, u32 port)
{
...
static void logic_reset(struct hl_cn_device *hdev, u32 port)
{
...
static void cpu_reset(struct hl_cn_device *hdev, u32 port)
{
...
static int fw_cmd(struct hl_cn_device *hdev, u32 port, u32 cmd, u32 *detail, u32 expected_res,
		  u32 *res_ptr)
{
...
static void clock_init(struct hl_cn_device *hdev, u32 port, int lane)
{
...
static void set_tx_taps(struct hl_cn_device *hdev, u32 port, int lane, s32 tx_pre2, s32 tx_pre1,
...
static void init_pam4_tx(struct hl_cn_device *hdev, u32 port, int lane)
{
...
static void init_pam4_rx(struct hl_cn_device *hdev, u32 port, int lane)
{
	/* ac-couple always */
...
static void set_lane_mode_tx(struct hl_cn_device *hdev, u32 port, int lane, bool pam4)
{
	if (pam4) {
		/* Disable NRZ mode */
...
static void set_lane_mode_rx(struct hl_cn_device *hdev, u32 port, int lane, bool pam4)
{
	if (pam4)
		/* Enable PAM4 mode */
...
static void prbs_mode_select_tx(struct hl_cn_device *hdev, u32 port, int lane, bool pam4,
				char *mode)
{
	u32 val;
...
static void prbs_mode_select_rx(struct hl_cn_device *hdev, u32 port, int lane, bool pam4,
				char *mode)
{
	u32 val;
...
static void set_default_polarity_values(struct hl_cn_device *hdev)
{
	...
	cpucp_info->pol_tx_mask[0] = pol_tx;
	cpucp_info->pol_rx_mask[0] = pol_rx;
}

static void set_default_mac_lane_remap(struct hl_cn_device *hdev)
{
...
static s32 get_pam4_tap_post2(struct hl_cn_device *hdev, u32 card_location, u32 abs_lane_idx)
{
	return 0;
}

static void set_default_tx_taps_values(struct hl_cn_device *hdev)
{
...
static void set_pol_tx(struct hl_cn_device *hdev, u32 port, int lane, u32 tx_pol)
{
	NIC_PHY_RMWREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PA0, tx_pol,
			      NIC0_SERDES0_LANE0_REGISTER_0PA0_TX_ANA_OUT_FLIP_MASK);
}

static void set_pol_rx(struct hl_cn_device *hdev, u32 port, int lane, u32 rx_pol)
{
...
static void init_lane_for_fw_rx(struct hl_cn_device *hdev, u32 port, int lane, bool pam4,
				bool do_lt)
{
	struct hl_cn_cpucp_info *cpucp_info = hdev->cpucp_info;
	...
	abs_lane_idx = (port << 1) + lane;
	rx_pol = (cpucp_info->pol_rx_mask[0] >> abs_lane_idx) & 1;
...
static void set_functional_mode_lane(struct hl_cn_device *hdev, u32 port, int lane, bool do_lt)
{
...
static void set_functional_mode(struct hl_cn_device *hdev, u32 port)
{
	struct hl_cn_port *cn_port = &hdev->cn_ports[port];
	int lane, tx_lane;
	u32 tx_port;
...
static u32 get_fw_reg(struct hl_cn_device *hdev, u32 port, u32 fw_addr)
{
	u32 ignore;
...
static int set_fw_reg(struct hl_cn_device *hdev, u32 port, u32 fw_addr, u32 val)
{
	u32 ignore;
...
static void enable_lane_swapping(struct hl_cn_device *hdev, u32 port, int lane, bool do_an,
				 bool do_lt)
{
	if (do_an || do_lt)
...
static void disable_lane_swapping(struct hl_cn_device *hdev, u32 port, int lane, bool do_an,
				  bool do_lt)
{
	if (do_an)
...
static void lane_swapping_config(struct hl_cn_device *hdev, u32 port, int lane, bool do_an,
				 bool do_lt)
{
...
static int fw_start(struct hl_cn_device *hdev, u32 port, int lane, bool pam4, bool do_lt)
{
...
static int fw_start_tx(struct hl_cn_device *hdev, u32 port, int lane, bool pam4, bool do_lt)
{
...
static int fw_config_vcocap(struct hl_cn_device *hdev, u32 port, int lane, u32 mode,
			    u32 counter_value)
{
	u32 ignore;
...
static int set_pll_tx(struct hl_cn_device *hdev, u32 port, int lane, u32 data_rate)
{
...
static int set_pll_rx(struct hl_cn_device *hdev, u32 port, int lane, u32 data_rate)
{
...
static int set_pll(struct hl_cn_device *hdev, u32 port, int lane, u32 data_rate)
{
...
static void set_tx_taps_scale(struct hl_cn_device *hdev, u32 port, int lane)
{
	NIC_PHY_RMWREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PAF, 0x4, 0x3E);
}

static int fw_config_speed_pam4(struct hl_cn_device *hdev, u32 port, int lane, bool do_lt)
{
...
static void init_nrz_tx(struct hl_cn_device *hdev, u32 port, int lane)
{
	NIC_PHY_WREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PA1, 0x0);
	NIC_PHY_WREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PA2, 0x0);
	NIC_PHY_WREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PA3, 0x0);
...
static void init_nrz_rx(struct hl_cn_device *hdev, u32 port, int lane)
{
	NIC_PHY_WREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PF8, 0xEC06);
}

static int fw_config_speed_nrz(struct hl_cn_device *hdev, u32 port, int lane)
{
...
static void reset_mac_tx(struct hl_cn_device *hdev, u32 port)
{
...
static int fw_config(struct hl_cn_device *hdev, u32 port, u32 data_rate, bool do_lt)
{
	u32 card_location;
	int lane, rc;
	bool pam4;
...
static void phy_port_reset(struct hl_cn_device *hdev, u32 port)
{
	int lane;
...
static void prbs_reset(struct hl_cn_port *cn_port, int lane, bool pam4)
{
...
static u64 _get_prbs_cnt(struct hl_cn_port *cn_port, int lane, bool pam4)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	u32 port = cn_port->port;
	u64 cnt;
...
static enum lane_state get_prbs_cnt(struct hl_cn_port *cn_port, int lane, bool pam4,
...
static void _calc_ber_lane(struct hl_cn_port *cn_port, int lane, u64 total_cnt, u64 error_cnt,
			   struct hl_cn_ber_info *ber_info)
{
	...
	total_num_digits = hl_cn_get_num_of_digits(total_cnt);
	error_num_digits = hl_cn_get_num_of_digits(error_cnt);
...
static void calc_ber_lane(struct hl_cn_port *cn_port, int lane, bool pam4)
{
...
static void calc_ber(struct hl_cn_port *cn_port)
{
	int lane;
...
static void check_pcs_link(struct hl_cn_port *cn_port)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	...
	if (cn_port->pcs_remote_fault_seq_cnt == NIC_PHY_MAC_REMOTE_FAULT_CNT) {
		dev_dbg(hdev->dev,
			"Card %u Port %u: got %d sequential remote faults\n",
	...
	if (cn_port->retry_cnt == NIC_PHY_PCS_LINK_CNT) {
		dev_dbg(hdev->dev,
			"Card %u Port %u: %d sequential seconds of PCS link down - reconfiguring PHY\n",
			card_location, port, NIC_PHY_PCS_LINK_CNT);

		hl_cn_phy_port_reconfig(cn_port);
	}
}

static u32 rv_debug(struct hl_cn_device *hdev, u32 port, int lane, u32 mode, u32 index)
{
	u32 cmd, res;
...
static int fw_tuning(struct hl_cn_device *hdev, u32 port, int lane, bool pam4)
{
	u32 state, mode;
...
static void do_fw_tuning(struct hl_cn_port *cn_port)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	...
	if (!rc) {
		...
		if (cn_port->eq_pcs_link) {
			...
			hl_cn_phy_set_port_status(cn_port, true);
		}
...
static int fw_tuning_an(struct hl_cn_device *hdev, u32 port, int lane)
{
...
static void tx_quite(struct hl_cn_device *hdev, u32 port, int lane)
{
	NIC_PHY_WREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PA1, 0x0);
	NIC_PHY_WREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PA2, 0x0);
	NIC_PHY_WREG32_LANE(NIC0_SERDES0_LANE0_REGISTER_0PA3, 0x0);
...
static int do_anlt(struct hl_cn_port *cn_port)
{
	struct hl_cn_device *hdev = cn_port->hdev;
...
static void do_fw_tuning_auto_neg(struct hl_cn_port *cn_port)
{
	if (cn_port->auto_neg_enable) {
		if (do_anlt(cn_port))
			return;
	...
	cn_port->fw_tuning_limit_ts = ktime_add_ms(ktime_get(), NIC_PHY_FW_TUNING_TIMEOUT_MS);
...
void gaudi2_cn_phy_link_status_work(struct work_struct *work)
{
	...
	cn_port = container_of(work, struct hl_cn_port, link_status_work.work);
	hdev = cn_port->hdev;
...
static void set_tx(struct hl_cn_device *hdev, u32 port, int lane, bool enable)
{
...
void gaudi2_cn_phy_port_start_stop(struct hl_cn_port *cn_port, bool is_start)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	u32 port = cn_port->port, tx_port;
	int lane, tx_lane;
...
static int fw_start_an(struct hl_cn_device *hdev, u32 port, int lane)
{
	u32 detail = 0, ignore;
...
static int fw_start_an_tx(struct hl_cn_device *hdev, u32 port, int lane)
{
	u32 detail = 0, ignore;
	int rc;
...
static int fw_config_auto_neg(struct hl_cn_device *hdev, u32 port, int lane)
{
	struct hl_cn_port *cn_port = &hdev->cn_ports[port];
	...
	card_location = hdev->card_location;
	pflags = hl_cn_get_pflags(cn_port);
...
int gaudi2_cn_phy_port_power_up(struct hl_cn_port *cn_port)
{
	struct hl_cn_device *hdev = cn_port->hdev;
...
void gaudi2_cn_phy_port_reconfig(struct hl_cn_port *cn_port)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	u32 card_location, port;
	int rc;
...
int gaudi2_cn_phy_port_init(struct hl_cn_port *cn_port)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	u32 port = cn_port->port;
	int rc;
...
void gaudi2_cn_phy_port_fini(struct hl_cn_port *cn_port)
{
	hl_cn_phy_fini(cn_port);
}

int gaudi2_cn_phy_reset_macro(struct hl_cn_macro *cn_macro)
{
...
void gaudi2_cn_phy_flush_link_status_work(struct hl_cn_device *hdev)
{
	struct hl_cn_port *cn_port;
	int i;
...
static int find_first_enabled_port(struct hl_cn_device *hdev, u32 *port)
{
	int i;
...
static void fw_write_all(struct hl_cn_device *hdev, u32 addr, u32 data)
{
	int port;
...
static void fw_write_all_lanes(struct hl_cn_device *hdev, u32 addr, u32 data)
{
	int port, lane;
...
static void fw_unload_all(struct hl_cn_device *hdev)
{
	u32 port;
...
static u32 fw_crc(struct hl_cn_device *hdev, u32 port)
{
...
static u32 fw_hash(struct hl_cn_device *hdev, u32 port)
{
...
static int mcu_cal_enable_all(struct hl_cn_device *hdev)
{
	u32 port;
	int rc;
...
int gaudi2_cn_phy_fw_load_all(struct hl_cn_device *hdev)
{
...
u16 gaudi2_cn_phy_get_crc(struct hl_cn_device *hdev)
{
	u32 port;
	int rc;
...
static bool is_old_phy_fw_loaded(struct hl_cn_device *hdev)
{
	return gaudi2_cn_phy_get_crc(hdev) == 0x1723;
}

static bool is_phy_fw_with_anlt_support(struct hl_cn_device *hdev)
{
	return gaudi2_cn_phy_get_crc(hdev) == 0x185E;
}

int gaudi2_cn_phy_init(struct hl_cn_device *hdev)
{
	if (!hdev->phy_config_fw)
		return 0;
	 ...
	 */
	if (hdev->phy_load_fw && is_phy_fw_with_anlt_support(hdev))
...
static int fw_read_s16(struct hl_cn_device *hdev, u32 port, u32 offset)
{
...
static void get_channel_estimation_params(struct hl_cn_device *hdev, u32 port, int lane, u32 *of,
					  u32 *hf)
{
	struct hl_cn_port *cn_port = &hdev->cn_ports[port];
...
static void get_tx_taps(struct hl_cn_device *hdev, u32 port, int lane, int *tx_taps)
{
...
static void dump_ber_info(struct hl_cn_device *hdev, u32 port, int lane, char *buf, ssize_t size)
{
	struct hl_cn_ber_info *ber_info;
	u32 abs_lane_idx;
...
void gaudi2_cn_phy_dump_serdes_params(struct hl_cn_device *hdev, char *buf, size_t size)
{
	...
	for (lane = 0; lane < 2; lane++) {
		...
		__snprintf(buf, size,
			   "sd: %u\nphy_ready: %u\nppm: %d\nch_est_of: %u\nch_est_hf: %u\n"
			   "adaptation state: 0x%x\n", sd, phy_ready, ppm, ch_est_of, ch_est_hf,
			   adapt_state);
