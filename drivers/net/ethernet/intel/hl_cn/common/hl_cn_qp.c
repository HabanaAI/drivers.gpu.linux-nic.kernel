 ...
 * operation.
 */
static const enum hl_cn_qp_state_op qp_valid_state_op[CN_QP_NUM_STATE][CN_QP_NUM_STATE] = {
	[CN_QP_STATE_RESET] = {
...
static char *cn_qp_state_2name(enum hl_cn_qp_state state)
{
...
static inline int wait_for_qpc_idle(struct hl_cn_port *cn_port, struct hl_cn_qp *qp, bool is_req)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_asic_port_funcs *port_funcs;
	struct hl_cn_qpc_attr qpc_attr;
	int i, rc;

	/* SW-63650: do not wait for idle in simulator as qpc reads are very expensive */
...
static int cn_qp_op_reset(struct hl_cn_port *cn_port, struct hl_cn_qp *qp)
{
	struct hl_cn_device *hdev = cn_port->hdev;
...
static int cn_qp_op_reset_2init(struct hl_cn_port *cn_port, struct hl_cn_qp *qp)
{
...
static int cn_qp_op_2rts(struct hl_cn_port *cn_port, struct hl_cn_qp *qp,
			 struct hl_cni_req_conn_ctx_in *in)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_asic_funcs *asic_funcs;
	int rc;
...
static int cn_qp_op_2rtr(struct hl_cn_port *cn_port, struct hl_cn_qp *qp,
			 struct hl_cni_res_conn_ctx_in *in)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_asic_funcs *asic_funcs;
	int rc;
...
static inline int cn_qp_invalidate_qpc(struct hl_cn_port *cn_port, struct hl_cn_qp *qp, bool is_req)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_asic_funcs *asic_funcs;
	int i, rc;
...
static int cn_qp_invalidate(struct hl_cn_port *cn_port, struct hl_cn_qp *qp, bool is_req,
			    bool wait_for_idle)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	int rc;
	...
	rc = wait_for_qpc_idle(cn_port, qp, is_req);
	if (rc) {
...
/* Drain the Requester */
static int cn_qp_op_rts_2sqd(struct hl_cn_port *cn_port, struct hl_cn_qp *qp, void *attr)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_qpc_drain_attr *drain = attr;
	int rc = 0;
 ...
 */
static int cn_qp_op_sqd_2sqd(struct hl_cn_port *cn_port, struct hl_cn_qp *qp, void *attr)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_qpc_drain_attr *drain = attr;
...
static int cn_qp_op_2qpd(struct hl_cn_port *cn_port, struct hl_cn_qp *qp, void *attr)
{
	struct hl_cn_qpc_drain_attr *drain = attr;
	int rc = 0;
...
static int cn_qp_op_2reset(struct hl_cn_port *cn_port, struct hl_cn_qp *qp,
			   const struct hl_cn_qpc_reset_attr *attr)
{
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_asic_funcs *asic_funcs;
	struct hl_cn_qpc_drain_attr drain;
...
{
	enum hl_cn_qp_state prev_state = qp->curr_state;
	struct hl_cn_device *hdev = cn_port->hdev;
	struct hl_cn_asic_port_funcs *port_funcs;
	enum hl_cn_qp_state_op op;
	int rc;
