...
static int hl_cn_cmd_port_check(struct hl_cn_device *hdev, u32 port, u32 flags);
...
int hl_cn_read_spmu_counters(struct hl_cn_port *cn_port, u64 out_data[], u32 *num_out_data)
{
	...
	/* this function can be called from ethtool and FW status thread */
...
static int hl_cn_ib_verify_qp_id(struct hl_aux_dev *aux_dev, u32 qp_id, u32 port)
{
	struct hl_cn_asic_port_funcs *port_funcs;
	struct hl_cn_port *cn_port;
...
static void hl_cn_ib_aux_drv_fini(struct hl_cn_device *hdev)
{
	...
	if (!hdev->is_ib_aux_dev_initialized)
		return;
		    ...
		    struct hl_cni_alloc_conn_in *in, struct hl_cni_alloc_conn_out *out)
{
	...
	rc = hl_cn_cmd_port_check(hdev, port, NIC_PORT_CHECK_OPEN | NIC_PORT_PRINT_ON_ERR);
	if (rc)
		return rc;
...
static void qp_destroy_work(struct work_struct *work)
{
	 ...
	 * Lock is to avoid concurrent memory access from a new handle created before freeing memory
	 */
	port_funcs->cfg_unlock(cn_port);
...
static int destroy_qp(struct hl_cn_device *hdev, struct hl_cni_destroy_conn_in *in)
{
	...
	rc = hl_cn_cmd_port_check(hdev, port, flags);
	if (rc)
		return rc;
			     ...
			     struct hl_cn_ctx *ctx)
{
	 ...
	 */
	rc = hl_cn_cmd_port_check(hdev, port, NIC_PORT_CHECK_ENABLE | NIC_PORT_PRINT_ON_ERR);
	if (rc)
		return rc;
		   ...
		   struct hl_cni_eq_poll_in *in, struct hl_cni_eq_poll_out *out)
{
	...
	port = in->port;
	rc = hl_cn_cmd_port_check(hdev, port, NIC_PORT_PRINT_ON_ERR);
	if (rc)
		return rc;
...
static int user_db_fifo_unset(struct hl_cn_device *hdev, struct hl_cni_user_db_fifo_unset_in *in)
{
	...
	rc = hl_cn_cmd_port_check(hdev, in->port, NIC_PORT_CHECK_OPEN | NIC_PORT_PRINT_ON_ERR);
	if (rc)
		return rc;
