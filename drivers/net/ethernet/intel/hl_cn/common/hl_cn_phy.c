...
static void port_reset_state(struct hl_cn_port *cn_port)
{
	kfifo_reset(&cn_port->pcs_fail_fifo);
...
static u32 get_data_rate(struct hl_cn_port *cn_port)
{
...
void hl_cn_phy_set_port_status(struct hl_cn_port *cn_port, bool up)
{
	...
	/* TODO: SW-166227: add delay to workaround EDK crash */
	if (hdev->pldm) {
		dev_dbg(hdev->dev, "port:%u hl_cn_phy_set_port_status\n", cn_port->port);
		msleep(1000);
	}
}
